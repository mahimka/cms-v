...
namespace :snap_shots do
  ...
  desc "Разобрать SnapShot ресторанов tripadvisor.com своим Nokogiri-парсером, без AI (rake snap_shots:parse_tripadvisor_restaurants [id=4] [limit=10])"
  task :parse_tripadvisor_restaurants do
    parser = SnapShotParser.new(client: TripadvisorRestaurantParser.new)
