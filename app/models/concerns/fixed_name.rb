# List#conditions ссылается на теги/схемы по name (строковый ключ),
# а не по id (id нестабилен между реимпортами). Если это поле можно
# менять свободно, любая ссылка из List молча перестаёт находить
# объект. fixed: true — явный признак "на это значение уже кто-то
# ссылается, дальше руками не трогать".
module FixedName
